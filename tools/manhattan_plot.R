...
# Map log_pvalue to point size with conditional size scaling
all_data[, point_size := ifelse(log_pvalue <= 5, 1.5, scales::rescale(log_pvalue, to = c(1.5, 3), from = c(3, ylim_upper)))]

# Generate the Manhattan plot
manhattan_plot <- ggplot(all_data, aes(x = position, y = log_pvalue)) +
  geom_point(data = subset(all_data, !is.na(pvalue)), aes(shape = category, color = color_category, size = point_size, group = chromosome), alpha = 1) +
  geom_point(data = subset(all_data, is.na(pvalue)), shape = 1, color = "grey", alpha = 0, size = 1.5) + # Placeholder points
  geom_hline(yintercept = 7.3, color = "orangered",linetype="dashed", linewidth = 0.5) +
  scale_shape_manual(values = c("f_dot" = 16, "dot" = 1, "triangle" = 2, "dagger" = 3, "cross" = 4, "diamond" = 5)) +
